...
def parse_gps_coordinate(values: Any, reference: Any) -> float:
    ...
    if direction in {"S", "W"}:
        decimal_degrees *= -1
    if not math.isfinite(decimal_degrees):
        raise ValueError("EXIF GPS coordinate is not finite")
...
def build_heatmap(reports: list[dict[str, Any]]) -> folium.Map:
    ...
    for report in reports:
        report_name = html.escape(str(report["report"]))
        gps_source = html.escape(str(report["gps_source"]))
        popup_html = (
            f"<b>{report_name}</b><br>"
            f"Detected items: {report['detected_items']}<br>"
            f"Severity: {report['severity']}/5<br>"
            f"GPS source: {gps_source}<br>"
